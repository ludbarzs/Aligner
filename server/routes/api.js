...
router.post("/images/upload", upload.single("image"), async (req, res) => {
  try {
    ...
    const metadata = {
      ...
      cornerCoordinates: req.body.cornerCoordinates
        ? JSON.parse(req.body.cornerCoordinates)
        : {},
    ...
    const result = await databaseController.saveImage(
      imageData,
      userId,
      projectId || null,
      metadata,
...
router.post("/presets", async (req, res) => {
  try {
    ...
    if (!userId || !presetData) {
      return res
        .status(400)
        .json({ error: "userId and presetData are required" });
    }
...
router.put("/preferences/:userId", async (req, res) => {
  try {
    ...
    if (!userId || !preferences) {
      return res
        .status(400)
        .json({ error: "userId and preferences data are required" });
    }

    const result = await databaseController.updateUserPreferences(
      userId,
      preferences,
    );
    res.json({
