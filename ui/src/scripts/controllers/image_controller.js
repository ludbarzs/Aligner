...
export class ImageController {
   ...
   */
  static async saveCurrentState() {
    const user = await appwriteService.getCurrentUser();
    if (!user) {
      throw new Error("No user is currenlty logged in");
    }

    // Get the user local DB ID
    const response = await fetch(
      `${this.API_BASE_URL}/users/appwrite/${user.$id}`,
    );
    if (!response.ok) {
      throw new Error("Failed to get user ID from local DB");
    }
  ...
  static async getUserImages(userId) {
    try {
      const response = await fetch(
        `${this.API_BASE_URL}/images/user/${userId}`,
      );
      if (!response.ok) {
